...
- [x] **Firestore security rules**: per-hospital scoping for `patients`, `locations`, `checkouts`, `archives`, `staff`, `users`. Create `firestore.rules` + `firebase.json` and deploy.
...
- [x] **Metric cards**: replace hardcoded values with `getCountFromServer` count queries per hospital (total patients, archive balance, active checkouts). Time-range deltas from `createdAt`/`admissionDate` vs start of period. (Implemented `countPatients`/`countArchives`/`countCheckouts` + `getDashboardCounts`; `DashboardPage` wired with loading state + range picker.)
...
- [x] **Action Watchlist**: derive overdue rows from `checkouts` (past `expectedDischargeDate`, not completed/cancelled). (`ActionWatchlist` now derives rows from `queryCheckouts`.)
